...
plot_path <-  'result/arrhythmia.png'

label_path <-  'data/arrhythmia/label.csv'
user_path <-  'data/arrhythmia/user_results.csv'
AI_path <-  'data/arrhythmia/AI_pred.csv'
...
arrhymia_names <- colnames(label)[grepl('LABEL', colnames(label))] %>% gsub('LABEL\\[', '', .) %>% gsub('\\]', '', .)

roc_p_list <- list()

for (q in 1:length(arrhymia_names)) {
  
  ## Generate label and prediction
  
  label <- label[,colnames(label)[!colnames(label) %in% 'x']]
  label[,'y'] <- label[,paste0('LABEL[', arrhymia_names[q], ']')] 
  AI[,'x'] <- AI[,paste0('AI[', arrhymia_names[q], ']')] 
  label <- merge(label, unique(AI[,c('hash_id', 'x')]), by = 'hash_id', all.x = TRUE)
  
  ## Generate user accuracy
  
  user <- user[,colnames(user)[!colnames(user) %in% 'y']]
  user[,'x'] <- user[,paste0('USER_ANSWER[', arrhymia_names[q], ']')] 
  ...
  roc_p <- roc_p + ggtitle(arrhymia_names[q]) + xlab('Specificity') + ylab('Sensitivity')
  ...
  roc_p <- roc_p + theme(plot.title = element_text(color = "#000000", size = 8),
...
merge_roc_p <- do.call('arrangeGrob', roc_p_list)

final_p <- ggdraw()
final_p <- final_p + draw_plot(merge_roc_p, x = 0, y = 0, width = 1, height = 1)
final_p <- final_p + draw_plot(legend_p, x = 0.25, y = 0, width = 0.1, height = 0.2)

png(filename = plot_path, width = 1750, height = 1750)
print(final_p)
dev.off()
