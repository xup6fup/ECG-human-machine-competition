...
plot_path <-  'result/pericarditis.png'

label_path <-  'data/pericarditis/label.csv'
user_path <-  'data/pericarditis/user_results.csv'
AI_path <-  'data/pericarditis/AI_pred.csv'
...
label[,'y'] <- apply(label[,c('LABEL[Pericarditis]'),drop = FALSE], 1, sum)
AI[,'x'] <- apply(AI[,c('AI[Pericarditis]'),drop = FALSE], 1, sum)
label <- merge(label, unique(AI[,c('hash_id', 'x')]), by = 'hash_id', all.x = TRUE)

## Generate user accuracy

user[,'x'] <- apply(user[,c('USER_ANSWER[Pericarditis]'),drop = FALSE], 1, sum)
...
roc_p <- roc_p + ggtitle('Pericarditis vs. not-Pericarditis') + xlab('Specificity') + ylab('Sensitivity')
