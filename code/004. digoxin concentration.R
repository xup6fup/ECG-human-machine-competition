...
plot_path <-  'result/digoxin concentration.png'

label_path <-  'data/digoxin concentration/label.csv'
user_path <-  'data/digoxin concentration/user_results.csv'
AI_path <-  'data/digoxin concentration/AI_pred.csv'
...
# Plot Digoxin toxicity

## Generate label and prediction

label[,'y'] <- apply(label[,c('LABEL[High risk]'),drop = FALSE], 1, sum)
AI[,'x'] <- apply(AI[,c('AI[High risk]'),drop = FALSE], 1, sum)
label <- merge(label, unique(AI[,c('hash_id', 'x')]), by = 'hash_id', all.x = TRUE)

## Generate user accuracy

user[,'x'] <- apply(user[,c('USER_ANSWER[High risk]'),drop = FALSE], 1, sum)
...
roc_p <- roc_p + ggtitle('Digoxin toxicity vs. not-Digoxin toxicity') + xlab('Specificity') + ylab('Sensitivity')
