...
plot_path <-  'result/pneumothorax.png'

label_path <-  'data/pneumothorax/label.csv'
user_path <-  'data/pneumothorax/user_results.csv'
AI_path <-  'data/pneumothorax/AI_pred.csv'
...
label[,'y'] <- apply(label[,c('LABEL[Left Pneumothorax]', 'LABEL[Right Pneumothorax]'),drop = FALSE], 1, sum)
AI[,'x'] <- apply(AI[,c('AI[Left Pneumothorax]', 'AI[Right Pneumothorax]'),drop = FALSE], 1, sum)
label <- merge(label, unique(AI[,c('hash_id', 'x')]), by = 'hash_id', all.x = TRUE)

## Generate user accuracy

user[,'x'] <- apply(user[,c('USER_ANSWER[Left Pneumothorax]', 'USER_ANSWER[Right Pneumothorax]'),drop = FALSE], 1, sum)
...
roc_p <- roc_p + ggtitle('Pneumothorax vs. not-Pneumothorax') + xlab('Specificity') + ylab('Sensitivity')
