...
plot_path <-  'result/pulmonary embolism.png'

label_path <-  'data/pulmonary embolism/label.csv'
user_path <-  'data/pulmonary embolism/user_results.csv'
AI_path <-  'data/pulmonary embolism/AI_pred.csv'
...
# Plot Pulmonary embolism

## Generate label and prediction

label[,'y'] <- apply(label[,c('LABEL[Acute Pulmonary Embolism]'),drop = FALSE], 1, sum)
AI[,'x'] <- apply(AI[,c('AI[Acute Pulmonary Embolism]'),drop = FALSE], 1, sum)
label <- merge(label, unique(AI[,c('hash_id', 'x')]), by = 'hash_id', all.x = TRUE)

## Generate user accuracy

user[,'x'] <- apply(user[,c('USER_ANSWER[Acute Pulmonary Embolism]'),drop = FALSE], 1, sum)
...
roc_p <- roc_p + ggtitle('Pulmonary embolism vs. not-Pulmonary embolism') + xlab('Specificity') + ylab('Sensitivity')
